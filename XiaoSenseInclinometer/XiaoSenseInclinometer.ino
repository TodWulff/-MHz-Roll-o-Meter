// INFO
  ...
  // ARMER REQUIRED CORE: MBED, IF USING THE DJI RC ARMING APP
  // INCLINOMETER ONLY:   EITHER MBED OR ARDUINO CORE CAN BE USED

// App Selection - uncomment >= 1 of the following as needed
...
// Uncomment the following to force the armer to not wait for DJI activity before jumping into main loop
 // this is useful if running the armer app alongside other apps, in this case, the inclinometer app
  #define enable_DJI_Activity_Override  

// Common serial comms related definess
  #define _comms_SUDelay            5000
  #define _comms_StartDelay         1000
  #define _comms_SerBaud            115200
  #define _comms_Ser1Baud           115200
...
// Xiao Inclinometer App
    ...
    // XIAO nRF52840 BLE SENSE INCLINOMETER PREPROCESSOR DIRECTIVES #############################
      ...
      // attitude caution/warning parameter setup ###############################################
        // these #defs are used to init variables which will, when implemented, be overwritten by values loaded from a config ini
        ...
        // BUFFERS for caution and warning (DEGREES): set these values based on user preference
        ...
        // TIP POINT ANGLES SETTINGS (DEGREES from Horizontal): set after the sensor is installed in a fully configured vehicle and tested for pitch and roll tip points
          ...
          #define _att_PosPitTip            75      // degrees - set to last whole degree where a nose up tipover doesn't happen
          #define _att_NegPitTip            -70     // degrees - set to last whole degree where a nose down tipover doesn't happen
          ...
          #define _att_PosRollTip           60      // degrees - set to last whole degree where a leaning-right rollover doesn't happen
          #define _att_NegRollTip           -60     // degrees - set to last whole degree where a leaning-left rollover doesn't happen

        // OFFSETS to correct orientation misalignment when installed on platform

          #define _att_PitchOffset          -1
          #define _att_RollOffset           3
      ...
      // Inclinometer Application #Includes #####################################################
    ...
    // XIAO nRF52840 BLE SENSE INCLINOMETER VARIABLE INSTANTIATION ##############################
      ...
      int pitchOffset = _att_PitchOffset;
      int rollOffset = _att_RollOffset;
    ...
    // XIAO nRF52840 BLE SENSE INCLINOMETER STARTUP PROCS #######################################
      void startDisplay() {             // init display via library procs
      ...
      #ifdef _INCLIN_DEBUG              // init comms if needed ISO debug
        void initComms() {                // init serial comms
            ...
            Serial.begin(_comms_SerBaud);
            while (!Serial && millis() < _comms_SUDelay);
              delay(_comms_StartDelay);
    ...
    // XIAO nRF52840 BLE SENSE INCLINOMETER APP PROCS ###########################################
      void updateOrientation() {	      // sense accelerations and do math to derive pitch/roll attitudes
        ...
        // convert radians to degrees, inverting if so needed, applying install offset corrections as needed
        lastPitch = pitchOffset + ((RAD_TO_DEG * (atan2(-xAng, -zAng) + PI)) * pDir);
        lastRoll = rollOffset + ((RAD_TO_DEG * (atan2(-yAng, -zAng) + PI)) * rDir);
      ...
      void smoothAttitude() {           // smooth derived pitch/roll attitudes by averaging over last _att_AvgSamples
        ...
        // these are already corrected for any installation offset in updateOrientation()
...
// DJI Armer App
    ...
    /* DJI ARMER APP PROCS *********************************************************************/
      void setup() {
        ...
        Serial1.begin(_comms_Ser1Baud);
        while (!Serial1 && millis() < _comms_SUDelay);
        delay(_comms_StartDelay);
      ...
      void loop() { 
        ...
        #ifdef enable_DJI_Activity_Override
          activityDetected = true;    // this is for testing to see if the DJI will arm upon power-up despite not waiting for DJI activity - UPDATE: IT DOES.
          #endif
      ...
      void resetArmer() {        // use WDT to implement a SWR
